...
VAL_FILE=./configs/base2novel/pretrained_on_k400/hmdb51/bmpt_s1.yaml
VAL_FILES=($VAL_FILE)
output_path=./output_hmdb/hmdb51_base2novel_v1
...
VAL_FILE_NOVEL=./configs/base2novel/pretrained_on_k400/hmdb51/bmpt_s1.yaml
...
VAL_FILE_NOVEL=./configs/base2novel/pretrained_on_k400/hmdb51/bmpt_novel_eval.yaml
VAL_FILES=($VAL_FILE_NOVEL)
for split_path in "${VAL_FILES[@]}"; do
    PYTHONWARNINGS="ignore" CUDA_VISIBLE_DEVICES=1,3 python -m torch.distributed.launch \
     --nproc_per_node=2 --master_port=29601 main.py \
