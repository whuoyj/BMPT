...
def mmcv_collate(batch, samples_per_gpu=1):
    ...
    # Handle DataContainer objects specially
    if isinstance(batch[0], DataContainer):
        # Stack DataContainers together
        stacked = DataContainer(
            [sample.data for sample in batch],
            cpu_only=batch[0].cpu_only,
            stack=False,
            padding_value=batch[0].padding_value if hasattr(batch[0], 'padding_value') else 0
        )
        return stacked
